...
2. Use a list with month names in order
3. Access list using index (month - 1) for zero-based indexing
...
| Component | Purpose |
|-----------|---------|
| `int(input())` | Reads integer month number |
| `months = ["January", "February", ...]` | List of month names in order |
| `months[month - 1]` | Accesses month name (adjust for zero-based index) |
| `print()` | Outputs the month name |

## 📅 Month Mapping

| Number | List Index | Month Name |
|--------|------------|------------|
| 1 | 0 | January |
| 2 | 1 | February |
| 3 | 2 | March |
...
- **List approach** is concise and efficient
- **Zero-based indexing** requires `month - 1`
- **First letter uppercase** as stored in list
