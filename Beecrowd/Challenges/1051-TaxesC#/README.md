...
### Strategy
1. Read salary with 2 decimal places
2. Check salary range using if-else
...
| Component | Purpose |
|-----------|---------|
| `double.Parse(..., CultureInfo.InvariantCulture)` | Reads salary with proper decimal format |
...
| `ToString("F2", CultureInfo.InvariantCulture)` | Formats tax to 2 decimal places |
...
- **Use CultureInfo.InvariantCulture** for correct decimal parsing and formatting
- **Output format:** "R$ {tax:F2}" with space after R$
