...
| Salary Range | Readjustment Rate |
|--------------|-------------------|
| 0 - 400.00 | 15% |
| 400.01 - 800.00 | 12% |
| 800.01 - 1200.00 | 10% |
...
2. Determine percentage based on salary range using if-elif-else
3. Calculate increase amount: salary × percentage ÷ 100
4. Calculate new salary: salary + increase
5. Print all three values with required formatting

## 🔍 Code Explanation

| Component | Purpose |
|-----------|---------|
| `float(input())` | Reads salary |
| `if salary <= 400` | Checks first range (0-400) |
| `elif salary <= 800` | Checks second range (400.01-800) |
| `elif salary <= 1200` | Checks third range (800.01-1200) |
| `elif salary <= 2000` | Checks fourth range (1200.01-2000) |
| `else` | Handles salaries above 2000 |
